...
def ExportActions(op, armature, actions):
    scene = bpy.context.scene
    addon_prefs = bfu_basics.GetAddonPrefs()
    dirpath = os.path.join(bfu_utils.GetObjExportDir(armature), scene.bfu_anim_subfolder_name)
    ...
    #####################################################
    '''
    # Export actions like a animation or pose
    ...
    for targetAction in actions:
        ...
        MyAsset.asset_name = bfu_naming.get_animation_file_name(armature, targetAction, "")
        MyAsset.asset_global_scale = armature.bfu_export_global_scale
        MyAsset.folder_name = armature.bfu_export_folder_name
        MyAsset.asset_type = bfu_utils.GetActionType(targetAction)
        MyAsset.animation_start_frame = bfu_utils.GetDesiredActionStartEndTime(armature, targetAction)[0]
        MyAsset.animation_end_frame = bfu_utils.GetDesiredActionStartEndTime(armature, targetAction)[1]

        file: bfu_export_logs.BFU_OT_FileExport = MyAsset.files.add()
        file.file_name = bfu_naming.get_animation_file_name(armature, targetAction, "")
        ...
        filename = file.GetFileWithExtension()
