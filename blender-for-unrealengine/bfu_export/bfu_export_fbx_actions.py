...
def ExportActions(op, armature, actions):
    ...
    asset_class = bfu_assets_manager.bfu_asset_manager_utils.get_asset_class(armature, "SkeletalAnimation")
    ...
    for targetAction in actions:
        ...
        my_asset_log.asset_name = bfu_naming.get_animation_file_name(armature, targetAction, "")
        my_asset_log.asset_global_scale = armature.bfu_export_global_scale
        my_asset_log.folder_name = armature.bfu_export_folder_name
        my_asset_log.asset_type = bfu_utils.GetActionType(targetAction)
        my_asset_log.animation_start_frame = bfu_utils.GetDesiredActionStartEndTime(armature, targetAction)[0]
        my_asset_log.animation_end_frame = bfu_utils.GetDesiredActionStartEndTime(armature, targetAction)[1]
